...
*/
    ...
    function writeHTML():void{
        for (let k in Angebot){//string to number wandeln damit man
    ...
    function writeFielset(_kathegorie:Produkt):void{
        let fielset:HTMLFieldsetElement=document.createElement('fieldset');
    ...
    function writeInput():void{
        ...
        let kathegorie+=`<input type:"${Angebot[k].type}" name="${Angebot[k].kathegorie}" id="${Angebot[k].bezeichnung}" preis="${Angebot[k].preis}">
                <label for="${Angebot[k].bezeichnung}">${Angebot[k].bezeichnung}</label>
                <br>`;
        }
    }
